...
LIBRARY_PATH=/home/prices-deployment/nix-environment/prices_product_libraries
...
ssh -i "$PRIVATE_KEY_PATH" -o StrictHostKeyChecking=no $USERNAME@$INSTANCE_IP "sudo bash $VM_ROOT_FILES/setup_environment.sh $NGINX_CERTIFICATE_NAME --y"
...
ssh -i "$PRIVATE_KEY_PATH" -o StrictHostKeyChecking=no $USERNAME@$INSTANCE_IP "sudo unzip -o $VM_ROOT_FILES/prices_product_libraries.zip -d $LIBRARY_PATH"
