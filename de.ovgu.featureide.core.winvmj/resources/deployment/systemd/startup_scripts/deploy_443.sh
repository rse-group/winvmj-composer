...
database_setup() {
  ...
  echo "SELECT 'CREATE DATABASE `echo $product_name_full | tr . _`' WHERE NOT EXISTS (SELECT FROM pg_database WHERE datname = '`echo $product_name_full | tr . _`') \gexec" | psql "postgresql://$DB_USERNAME:$DB_PASSWORD@$DB_URL"
}
...
abort_deployment() {
  ...
  sudo systemctl daemon-reload
  sudo systemctl restart nginx
  echo "DROP DATABASE IF EXISTS ${PRODUCT_PREFIX}_product_$product_name;" | psql "postgresql://$DB_USERNAME:$DB_PASSWORD@$DB_URL"
